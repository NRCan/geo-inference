...
def equalize_adapthist_enhancement(
    ...
    Each chunk is [band == 1, height, width]. So, the contrast enhancement is applied on each band separately.

    Args:
        aoi_chunk (np.ndarray): The overlapped chunk of dask array.
        clip_limit (int): The contrast limit for localized changes in contrast.

    Returns:
        ready_np_array (np.ndarray)
    """
        ...
        ready_np_array = (
            equalize_clahe(
                ...
                grid_size=(
                    1,
                    1,
                ),  # set to (1, 1) to not break down the chunks to smaller chunks
            )
            .cpu()
            .numpy()[0]
        )
...
def runModel_full_neighbor(
    ...
    """
    This function is for running the model on the full neighbor (all the neighbors around the chunked data).
    ...
    if chunk_data.size > 0 and chunk_data is not None:
        try:
            ...
            tensor = torch.as_tensor(chunk_data[np.newaxis, ...]).to(model.device)
            prediction_output = np.empty(
                shape=(num_classes, chunk_data.shape[1], chunk_data.shape[2])
            )  # Create the output but empty
            # run the model
            with torch.no_grad():
                prediction_output = model(tensor.to(model.device)).cpu().numpy()[0]
            del tensor
