...
def geojson_to_px_gdf(
    geojson, im_path, geom_col="geometry", output_path=None, override_crs=False
):
    ...
    transformed_gdf = affine_transform_gdf(
        overlap_gdf, affine_obj=affine_obj, inverse=True, geom_col=geom_col
    )
...
def get_overlapping_subset(gdf, im=None, bbox=None, bbox_crs=None):
    ...
    if im is not None:
        ...
        bbox = transform_bounds(
            im.crs, check_crs(gdf.crs, return_rasterio=True), *im.bounds
        )
        bbox_crs = im.crs
...
def coco_categories_dict_from_df(
    ...
    rename_dict = {category_id_col: "id", category_name_col: "name"}
...
def df_to_coco_annos(
    ...
    if preset_categories is not None and category_col is None:
        ...
        if not include_other:
            logger.debug("Filtering out objects not contained in " " preset_categories")
            ...
            if "other" not in category_dict.keys():
                ...
                preset_categories.append(
                    {"id": other_id, "name": "other", "supercategory": "other"}
                )
    ...
    coco_annotations = temp_df.apply(
        _row_to_coco,
        axis=1,
        geom_col=geom_col,
    ...
    output_dict = {"annotations": coco_annotations, "categories": coco_categories}
